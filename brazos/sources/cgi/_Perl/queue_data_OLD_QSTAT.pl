...
my ($sshh) = join '', map { q( ').($_).q(') } map { /^(.*?):?(\d*)$/ } (( split /\s+/, ${ Local::Subs::CONFIG_MODULES('QUEUE_DATA') || {}}{'SSH'} ), undef )[0];
my ($cmmd) = map { q( ').($_).q(') } map { (length) ? do { /(?:^|\/)qstat$/ && !(/\s/) or die 'Invalid qstat executable path'; $_ } : q(qstat) }
	${ Local::Subs::CONFIG_MODULES('QUEUE_DATA') || {}}{'COMMAND'};
my ($ques) = map { q( ').($_).q(') } ( join ',', ( grep { (length) } map { split /[,\s]+/ } ${ Local::Subs::CONFIG_MODULES('QUEUE_DATA') || {}}{'QUEUES'} ));
...
my ($task) = q(queue); do { my ($pidf) = grep {(defined)} fork() or die '2: Error Forking Process ... Exiting';
	do { setpgrp(0,0); exec ( BASE_PATH.CGI_PATH.'/_Shell/'.($task).'.sh'.($sshh.$cmmd.$ques).' > ' .
...
print "\t\t\t".'<td class="mon_default" style="width:154px; height:16px;">Active Cores (Running)</td>'."\n";
print "\t\t\t".'<td class="mon_default" style="width:154px; height:16px;">Requested Cores (Queued)</td>'."\n";
print "\t\t\t".'<td class="mon_default" style="width:154px; height:16px;">Other Core States<br>(Held, Waiting, Exiting)</td>'."\n";
print "\t\t".'</tr>'."\n";
my ($i) = 0; for (@{ $$data{QUEUE_UTILIZATION}}) { my (@qlod) = @$_;
	$qlod[0] = Local::Subs::DEFAULT( $$nick{$qlod[0]}, (uc $qlod[0])); @qlod[1..4] = map { Local::Subs::COMMA($_) } @qlod[1..4];
