...
my ($sshh) = map { (length $$_[0]) ? q(ssh ).($$_[0]).q( ).((length $$_[1]) ? q(-p ).$$_[1].q( ) : q()) : q() } map {[ /^(.*?):?(\d*)$/ ]}
	(( split /\s+/, ${ Local::Subs::CONFIG_MODULES('QSTAT_DATA') || {}}{'SSH'} ), undef )[0];
my ($cmmd) = map { (length) ? do { /(?:^|\/)qstat$/ && !(/\s/) or die 'Invalid qstat executable path'; $_ } : q(qstat) }
        ${ Local::Subs::CONFIG_MODULES('QSTAT_DATA') || {}}{'COMMAND'};
( length ( my $ques = join ' ', ( my (@ques) = ( grep { (length) } map { split /[,\s]+/ } ${ Local::Subs::CONFIG_MODULES('QSTAT_DATA') || {}}{'QUEUES'} ))))
	or die 'Cannot Establish List of Job Queues for Module QSTAT_DATA';
...
my (%idex) = map {( $ques[$_] => $_ )} (0..(@ques-1)); my (@nams) = map { (length $$nick{$_}) ? ($$nick{$_}) : (uc) } (@ques); my (@xmpt);
	($xmpt[$idex{$_}] = 1) for ( grep { (length) } map { split /[,\s]+/ } ${ Local::Subs::CONFIG_MODULES('QSTAT_DATA') || {}}{'EXEMPT'} );
...
( my ($dtim) = map { my ($t) = my ($t) = Local::Subs::TAKE_FIVE($_);
                     ($t > Local::Subs::TIME_STAMP(q(QSTAT_DATA),$t)) ? $t : () }
	       grep { /^\d+$/ } ${ $$data{QSTAT_TIME}}[0][0] ) or exit;
 
open FHO, '>'.BASE_PATH.MON_PATH.'/LOGS/QSTAT_DATA/'.($dtim).'.dat'; select FHO;
print q().( join "\t", @$_ )."\n" for grep {(@$_ == 9)}
            map {[ grep {(defined)}
              $$_[0] =~ /^(\d+(?:-\d+)?)$/ ? q().$1 : (),
              (length $$_[1]) ? $$_[1] : (),
              $$_[2],
              $$_[3] = ${{ 'Q' => 0, 'R' => 1, 'C' => 2, 'E' => 3, 'S' => 4, 'H' => 5, 'W' => 6, 'T' => 7 }}{$$_[3]},
              $$_[4] =~ /^\d+:(\d+)$/ ? 0+$1 : ($$_[3]==0) ?
              $$_[5] =~ /^\d+:(\d+)$/ && 0+$1 : (),
...
print FHO Local::Subs::QUEUE_TABLE(0,$dtim,\%stll,\%idex,\@nams,\@xmpt);
close FHO;
