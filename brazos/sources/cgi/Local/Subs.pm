...
sub TIME_STAMP {
        ...
        my ($key,$tsp,$osp,%tsp) = ((((shift) =~ /^(\w+)$/, undef )[0]),(((shift) =~ /^(\d+)$/, undef )[0]));
	do {
             (-f $_) or system ('touch '.$_);
             ( open FHB, q(+<), $_ ) && ( flock FHB, LOCK_EX ) or return undef
        } for BASE_PATH.MON_PATH.'/TIMESTAMP/data.txt';
	seek FHB, 0, SEEK_SET;
        while (<FHB>) {
             /^(\w+) (\d+)$/ and $tsp{$1} = $2;
        } return (( scalar { %tsp }),( close FHB ))[0] unless (defined $key);
	$osp = $tsp{$key};
        do {
             $tsp{$key} = $tsp;
             seek FHB, 0, SEEK_SET;
...
# skip undef'd idex's
sub QUEUE_TABLE {
  my ($time,$qstt,$mode,$ltim,$stll,$idex,$nams,$xmpt,$dtim,$ndat) = ( time, [],
	&DEFAULT((0,1)[(shift)],0), ( map {( $_ - ($_ % 300))} &DEFAULT((shift),(time-300))),
      ( map { (ref eq q(HASH)) ? $_ : {}} (shift)), ( map { (ref eq q(HASH)) ? $_ : undef } (shift)),
      ( map { (ref eq q(ARRAY)) ? $_ : undef } (shift,shift)));
  do {
    my ($jobi,$rusr,$rque,$emod,$ncpu,$memu,$ptim,$rtim,$etim,$ctim) = @$_;
    ...
    my ($tque,$tmod,$rmod,$tjob) = map {( $$_[0][$rque], $$_[1][$emod], $$_[2][$emod], $$_[3] )} (
	        [($nams || []), [ qw( Queued Running Completed Exiting Held Suspended Waiting Transfer )], [ (0,1,1,1,2,2,0,0)],
		        ...
		        ( join q(:), map { ((q(0))x(3-(length $_))).($_) } ($jobi =~ /^(\d+):(\d+)$/)) ], )[$mode];
    my ($kusr) = &LAST_NAME_FIRST($tusr);
    my ($qtim) = ( sort { $a <=> $b } (0,(($ctim || $dtim) - $etim - $ptim )))[1]; # err ? $dtim not defined ? set to ltim ?
    ...
    for my $i (0..2) {
      my ($path) = $$qstt[$i] ||= [[undef,{},{},[],(undef)[0..5]],{}];
      for (([$kusr,$tusr],[$rque,$tque],[$emod,$tmod])[$i],[$jobi,$tjob],undef) {
	$$path[0][1]{$tusr}++;
        $$path[0][2]{$tque}++;
        $$path[0][3][$rmod]++;
        $$path[0][4]++;
	$$path[0][5] += $ncpu;
        ...
        $$path[0][9] += $qtim;
        $$path[0][10][$rmod] += $ncpu;
	$path = ${ $$path[1]}{$$_[0]} ||= [[$$_[1],{},{},[],(undef)[0..5]],{}] if (defined);
      } @{ $$path[0]}[3,4] = ($rmod,$tmod);
    }
  }
  ...
  my (@qnam) = @{ ([ qw( qstat QStat Queue SLURM)], [ qw( condorq CondorQ Universe Condor)])[$mode] };
  my $tble = q();
  $tble .= '<DIV id="'.($qnam[0]).
           '_data_anchor" style="z-index:0; display:block; position:static; height:auto; width:826px; margin:0px 0px; padding:0px; text-align:center; overflow:hidden;">'.
           "\n";
  ...
  $tble .= "\t\t\t\t".'<DIV style="float:left; clear:none; width:30%; margin:0px; padding:0px; text-align:center;">' .
           (( join '&nbsp;|&nbsp;', map { '<a href="wayback.'.($html).'?'.(( qw( qstat condorq))[$mode]).'_'.($ltim+$$_[1]).
           '" class="mon_default" target="WAYBACK">'.($$_[0]).'</a>' } ([q(Month),-30*24*3600],[q(Week),-7*24*3600],[q(Day),-24*3600],[q(Hour),-3600],[q(Back),-300])).
           q(&nbsp;<<)).'</DIV>'."\n";
  $tble .= "\t\t\t\t".'<DIV style="float:left; clear:none; width:40%; margin:0px; padding:0px; text-align:center;">'.($qnam[3]).' Queue Job Status ( ' .
           ( &DATE_FORMAT(&DEFAULT($dtim,$ltim),0,q(%Y-%m-%d %R CST))).' )</DIV>'."\n";
  $tble .= "\t\t\t\t".'<DIV style="float:left; clear:none; width:30%; margin:0px; padding:0px; text-align:center;">' .
	    (( map { (length) ? q(>>&nbsp;).($_) : q() } ( join '&nbsp;|&nbsp;', map { '<a href="wayback.'.($html).'?'.(( qw( qstat condorq))[$mode]).'_'.
           ($ltim+$$_[1]).'" class="mon_default" target="WAYBACK">'.($$_[0]).
           '</a>' } ( grep {($$_[1] <= ($time - $ltim))} ([q(Next),+300],[q(Hour),+3600],[q(Day),+24*3600],[q(Week),+7*24*3600],[q(Month),+30*24*3600]))))[0]).
           '</DIV>'."\n";
  $tble .= "\t\t\t".'</td>'."\n";
  $tble .= "\t\t".'</tr>'."\n";
  for my $i (0..2) {
    $tble .= "\t\t".'<tr id="'.($qnam[0]).'_data_sort_'.$i.'" class="mon_default" style="display:'.(( qw( block none none ))[$i]).
    ...
    $tble .= "\t\t\t\t\t".'</tr>'."\n";
    my ($j) = 0;
    for ( sort ${( \sub { $a <=> $b }, \sub { $a cmp $b } )[0+($i==0)]} keys %{ $$qstt[$i][1]}) {
      my ($path) = ${ $$qstt[$i][1]}{$_};
      $tble .= "\t\t\t\t\t".'<tr class="mon_default" id="'.($qnam[0]).'_data_select_'.$i.'_'.$j.'" style="display:block; color:black; background-color:#'.
               (($$path[0][3][2]) ? q(E62E2E) : ($$path[0][3][0]) ? q(F2D230) : q(3DCC3D)).'; cursor:pointer;" onClick="javascript:'.($qnam[1]).'DataToggle('.($i).','.($j).
               ');" onMouseOver="javascript:this.style.backgroundColor=\'#3BB9FF\';" onMouseOut="javascript:this.style.backgroundColor=\'#'.
               (($$path[0][3][2]) ? q(E62E2E) : ($$path[0][3][0]) ? q(F2D230) : q(3DCC3D)).'\';">'."\n";
      $tble .= "\t\t\t\t\t\t".'<td class="mon_default" style="width:'.(( qw( 146 92 92 ))[$i]).'px; height:16px; color:inherit; background-color:transparent;">'."\n";
      $tble .= "\t\t\t\t\t\t\t".'<DIV style="position:relative; left:0px; top:0px; width:'.(( qw( 146 92 92 ))[$i]).'px; height:16px; padding:0px; display:block;">'.
               (&DEFAULT($$path[0][0],q(-)))."\n";
      ...
      $tble .= "\t\t\t\t\t\t".'</td>'."\n";
      $tble .= join q(), (
                 "\t\t\t\t\t\t".'<td class="mon_default" style="width:146px; height:16px; color:inherit; background-color:transparent;">'.
                 (( map { (@$_>1) ? q(MIXED) : &DEFAULT($$_[0],q(-)) } [ keys %{ $$path[0][1]}])[0]).(($i==2) && ' ('.(&DEFAULT(&COMMA($$path[0][4]),q(-))).')').'</td>'."\n",
          "\t\t\t\t\t\t".'<td class="mon_default" style="width:92px; height:16px; color:inherit; background-color:transparent;">'.
                 (( map { (@$_>1) ? q(MIXED) : &DEFAULT($$_[0],q(-)) } [ keys %{ $$path[0][2]}])[0]).'</td>'."\n",
                 "\t\t\t\t\t\t".'<td class="mon_default" style="width:92px; height:16px; color:inherit; background-color:transparent;">'.
                 ( join ' / ', &COMMA(&DEFAULT($$path[0][3][1],0)), &COMMA($$path[0][4])).'</td>'."\n",
               )[@{ ([1,2],[2,0],[0,1])[$i]}];
      $tble .= "\t\t\t\t\t\t".'<td class="mon_default" style="width:82px; height:16px; color:inherit; background-color:transparent;">'.
               ( join ' / ', &COMMA(&DEFAULT($$path[0][10][1],0)), &COMMA($$path[0][5])).'</td>'."\n";
      $tble .= "\t\t\t\t\t\t".'<td class="mon_default" style="width:82px; height:16px; color:inherit; background-color:transparent;">'.
               ( &DATA_FORMAT_BYTES_HUMAN($$path[0][6],1)).'</td>'."\n";
      $tble .= "\t\t\t\t\t\t".'<td class="mon_default" style="width:82px; height:16px; color:inherit; background-color:transparent;">'.
               ( &TIME_FORMAT_SECONDS_HOURS($$path[0][7],1)).'</td>'."\n";
      $tble .= "\t\t\t\t\t\t".'<td class="mon_default" style="width:82px; height:16px; color:inherit; background-color:transparent;">'.
               ( &TIME_FORMAT_SECONDS_HOURS($$path[0][8],1)).'</td>'."\n";
      $tble .= "\t\t\t\t\t\t".'<td class="mon_default" style="width:82px; height:16px; color:inherit; background-color:transparent;">'.
               ( &TIME_FORMAT_SECONDS_HOURS($$path[0][9],1)).'</td>'."\n";
      $tble .= "\t\t\t\t\t".'</tr>'."\n";
      my ($k) = 0;
      for ( map {( shift @$_ )} sort { ($$a[1]<=>$$b[1]) || ($$a[2]<=>$$b[2]) }
            map {[ $_, /^(\d+)(?:(?:|-)(\d+))?$/ ]} keys %{ $$path[1]} ) {
        my ($path) = ${ $$path[1]}{$_};
        $tble .= "\t\t\t\t\t".'<tr class="mon_default" id="'.($qnam[0]).'_data_element_'.($i).'_'.($j).'_'.($k).'" style="display:none; color:black; background-color:#'.
                 (( qw( F5F384 F5E964 CBF7D2 9AE3A5 F5D0D0 F7B0B0 ))[(2*$$path[0][3])+($k%2)]).';">'."\n";
        #$tble .= "\t\t\t\t\t\t".'<td class="mon_default" style="position:relative; left:0px; top:0px; width:'.(( qw( 146 92 92 ))[$i]).
                 #'px; height:16px; color:inherit; background-color:transparent;">'.(( '<a href="http://brazos.tamu.edu/cgi-bin/jobstat.pl?'.($_).
                 #'" class="mon_default" target="NEW">', q())[$mode]).($$path[0][0]).( ('</a>')[$mode]).'</td>'."\n";
        $tble .= "\t\t\t\t\t\t".'<td class="mon_default" style="position:relative; left:0px; top:0px; width:'.(( qw( 146 92 92 ))[$i]).
                 'px; height:16px; color:inherit; background-color:transparent;">'.($$path[0][0]).'</td>'."\n";
        $tble .= join q(), (
                   "\t\t\t\t\t\t".'<td class="mon_default" style="width:146px; height:16px; color:inherit; background-color:transparent;">'.
                   (( map { &DEFAULT($$_[0],q(-)) } [ keys %{ $$path[0][1]}])[0]).'</td>'."\n",
                   "\t\t\t\t\t\t".'<td class="mon_default" style="width:92px; height:16px; color:inherit; background-color:transparent;">'.
                   (( map { &DEFAULT($$_[0],q(-)) } [ keys %{ $$path[0][2]}])[0]).'</td>'."\n",
                   "\t\t\t\t\t\t".'<td class="mon_default" style="width:92px; height:16px; color:inherit; background-color:transparent;">'.
                   (&DEFAULT($$path[0][4],q(-))).'</td>'."\n",
                 )[@{ ([1,2],[2,0],[0,1])[$i]}];
        $tble .= "\t\t\t\t\t\t".'<td class="mon_default" style="width:82px; height:16px; color:inherit; background-color:transparent;">'.( &COMMA($$path[0][5])).'</td>'."\n";
        $tble .= "\t\t\t\t\t\t".'<td class="mon_default" style="width:82px; height:16px; color:inherit; background-color:transparent;">'.
                 ( &DATA_FORMAT_BYTES_HUMAN($$path[0][6],1)).'</td>'."\n";
        $tble .= "\t\t\t\t\t\t".'<td class="mon_default" style="width:82px; height:16px; color:inherit; background-color:transparent;">'.
                 ( &TIME_FORMAT_SECONDS_HOURS($$path[0][7],2)).'</td>'."\n";
        $tble .= "\t\t\t\t\t\t".'<td class="mon_default" style="width:82px; height:16px; color:inherit; background-color:transparent;">'.
                 ( &TIME_FORMAT_SECONDS_HOURS($$path[0][8],2)).'</td>'."\n";
        $tble .= "\t\t\t\t\t\t".'<td class="mon_default" style="width:82px; height:16px; color:inherit; background-color:transparent;">'.
                 ( &TIME_FORMAT_SECONDS_HOURS($$path[0][9],2)).'</td>'."\n";
        $tble .= "\t\t\t\t\t".'</tr>'."\n";
      } continue { $k++; }
    } continue { $j++; }
    $tble .= "\t\t\t\t\t".'<tr class="mon_default" style="display:block; color:black; background-color:#20B2AA;">'."\n";
    ...
    $tble .= join q(), (
               "\t\t\t\t\t\t".'<td class="mon_default" style="width:146px; height:16px; color:inherit; background-color:transparent;">'.
               (( map { (@$_>1) ? q(MIXED) : &DEFAULT($$_[0],q(-)) } [ keys %{ $$qstt[$i][0][1]}])[0]).(($i==2) && ' ('.(&DEFAULT(&COMMA($$qstt[$i][0][4]),q(-))).
               ')').'</td>'."\n",
               "\t\t\t\t\t\t".'<td class="mon_default" style="width:92px; height:16px; color:inherit; background-color:transparent;">'.
               (( map { (@$_>1) ? q(MIXED) : &DEFAULT($$_[0],q(-)) } [ keys %{ $$qstt[$i][0][2]}])[0]).'</td>'."\n",
               "\t\t\t\t\t\t".'<td class="mon_default" style="width:92px; height:16px; color:inherit; background-color:transparent;">'.( join ' / ',
               ((defined $$qstt[$i][0][4]) ? ( &COMMA(&DEFAULT($$qstt[$i][0][3][1],0)), &COMMA($$qstt[$i][0][4])) : ( q(-), q(-)))).'</td>'."\n",
             )[@{ ([1,2],[2,0],[0,1])[$i]}];
    $tble .= "\t\t\t\t\t\t".'<td class="mon_default" style="width:82px; height:16px; color:inherit; background-color:transparent;">'.( join ' / ',
             ((defined $$qstt[$i][0][5]) ? ( &COMMA(&DEFAULT($$qstt[$i][0][10][1],0)), &COMMA($$qstt[$i][0][5])) : (q(-), q(-)))).'</td>'."\n";
    $tble .= "\t\t\t\t\t\t".'<td class="mon_default" style="width:82px; height:16px; color:inherit; background-color:transparent;">'.
             ((defined $$qstt[$i][0][6]) ? &DATA_FORMAT_BYTES_HUMAN($$qstt[$i][0][6],1) : q(-)).'</td>'."\n";
    $tble .= "\t\t\t\t\t\t".'<td class="mon_default" style="width:82px; height:16px; color:inherit; background-color:transparent;">'.
             ((defined $$qstt[$i][0][7]) ? &TIME_FORMAT_SECONDS_HOURS($$qstt[$i][0][7],1) : q(-)).'</td>'."\n";
    $tble .= "\t\t\t\t\t\t".'<td class="mon_default" style="width:82px; height:16px; color:inherit; background-color:transparent;">'.
             ((defined $$qstt[$i][0][8]) ? &TIME_FORMAT_SECONDS_HOURS($$qstt[$i][0][8],1) : q(-)).'</td>'."\n";
    $tble .= "\t\t\t\t\t\t".'<td class="mon_default" style="width:82px; height:16px; color:inherit; background-color:transparent;">'.
             ((defined $$qstt[$i][0][9]) ? &TIME_FORMAT_SECONDS_HOURS($$qstt[$i][0][9],1) : q(-)).'</td>'."\n";
    $tble .= "\t\t\t\t\t".'</tr>'."\n";
    $tble .= "\t\t\t\t".'</table>'."\n";
    $tble .= "\t\t\t".'</td>'."\n";
    $tble .= "\t\t".'</tr>'."\n";
  ...
  $tble .= "\t\t\t".'<td class="mon_default" style="width:350px; height:16px; color:inherit; background-color:#FFFFFF;' .
	   (( keys %{ $$qstt[0][1]}) ? ' cursor:pointer;" onClick="javascript:'.($qnam[1]).
           'DataToggleAll();" onMouseOver="javascript:this.style.backgroundColor=\'#3BB9FF\';" onMouseOut="javascript:this.style.
           backgroundColor=\'#FFFFFF\';">' : '">' ) . (( keys %{ $$qstt[0][1]}) ? '&uarr; Click to Expand or Collapse Table' : 'No Jobs Queued' ) . '</td>'."\n";
  $tble .= "\t\t\t".'<td class="mon_default" style="width:174px; height:16px;">Select Sorting By &rarr;</td>'."\n";
  $tble .= "\t\t\t".'<td id="'.($qnam[0]).
           '_data_sort_select_user" class="mon_default" style="width:82px; height:16px; color:black; background-color:#3BB9FF; cursor:pointer;" onClick="javascript:'.
           ($qnam[1]).'DataSortSelect(0);" onMouseOver="javascript:this.style.backgroundColor=\'#3BB9FF\';" onMouseOut="javascript:if('.($qnam[1]).
           'DataSortIndex!=0)this.style.backgroundColor=\'#FFFFFF\';">User</td>'."\n";
  $tble .= "\t\t\t".'<td id="'.($qnam[0]).
           '_data_sort_select_queue" class="mon_default" style="width:82px; height:16px; color:black; background-color:#FFFFFF; cursor:pointer;" onClick="javascript:'.
           ($qnam[1]).'DataSortSelect(1);" onMouseOver="javascript:this.style.backgroundColor=\'#3BB9FF\';" onMouseOut="javascript:if('.($qnam[1]).
           'DataSortIndex!=1)this.style.backgroundColor=\'#FFFFFF\';">'.($qnam[2]).'</td>'."\n";
  $tble .= "\t\t\t".'<td id="'.($qnam[0]).
           '_data_sort_select_status" class="mon_default" style="width:82px; height:16px; color:black; background-color:#FFFFFF; cursor:pointer;" onClick="javascript:'.
           ($qnam[1]).'DataSortSelect(2);" onMouseOver="javascript:this.style.backgroundColor=\'#3BB9FF\';" onMouseOut="javascript:if('.($qnam[1]).
           'DataSortIndex!=2)this.style.backgroundColor=\'#FFFFFF\';">Status</td>'."\n";
  $tble .= "\t\t".'</tr>'."\n";
...
# closure for stll tests
# skip undef'd idex's
# # THIS IS FOR SWITCHING REPORTING OF HEPX JOBS IN THE QUEUES FROM QSTAT TO SQUEUE
sub SQUEUE_TABLE {
  my ($time,$qstt,$mode,$ltim,$stll,$idex,$nams,$xmpt,$dtim,$ndat) = ( time, [],
	&DEFAULT((0,1)[(shift)],0), ( map {( $_ - ($_ % 300))} &DEFAULT((shift),(time-300))),
      ( map { (ref eq q(HASH)) ? $_ : {}} (shift)), ( map { (ref eq q(HASH)) ? $_ : undef } (shift)),
  ...
  for map { ( open FHI, $_ ) ? @{ (( grep { $dtim = $ltim; 1 } [ map { chomp; [ split "\t" ]} <FHI> ]), ( close FHI ))[0] } : () }
    grep { ( -f ) || ($ndat++) } BASE_PATH.MON_PATH.'/LOGS/'.(( qw( SQUEUE CONDORQ))[$mode]).'_DATA/'.($ltim).'.dat';
  my ($html) = &HTML_EXTENSION();
  my (@qnam) = @{ ([ qw( squeue Squeue Queue SLURM)], [ qw( condorq CondorQ Universe Condor)])[$mode] };
  my $tble = q();
  $tble .= '<DIV id="'.($qnam[0]).
           '_data_anchor" style="z-index:0; display:block; position:static; height:auto; width:826px; margin:0px 0px; padding:0px; text-align:center; overflow:hidden;">'.
           "\n";
  ...
  $tble .= "\t\t\t".'<td colspan="5" class="mon_default" style="width:810px; height:16px;">'."\n";
  $tble .= "\t\t\t\t".'<DIV style="float:left; clear:none; width:30%; margin:0px; padding:0px; text-align:center;">' .
           (( join '&nbsp;|&nbsp;', map { '<a href="wayback.'.($html).'?'.(( qw( squeue condorq))[$mode]).'_'.($ltim+$$_[1]).
           '" class="mon_default" target="WAYBACK">'.($$_[0]).'</a>' } ([q(Month),-30*24*3600],[q(Week),-7*24*3600],[q(Day),-24*3600],[q(Hour),-3600],[q(Back),-300])).
           q(&nbsp;<<)).'</DIV>'."\n";
  $tble .= "\t\t\t\t".'<DIV style="float:left; clear:none; width:40%; margin:0px; padding:0px; text-align:center;">'.($qnam[3]).' Queue Job Status ( ' .
           ( &DATE_FORMAT(&DEFAULT($dtim,$ltim),0,q(%Y-%m-%d %R CST))).' )</DIV>'."\n";
  $tble .= "\t\t\t\t".'<DIV style="float:left; clear:none; width:30%; margin:0px; padding:0px; text-align:center;">' .
	    (( map { (length) ? q(>>&nbsp;).($_) : q() } ( join '&nbsp;|&nbsp;', map { '<a href="wayback.'.($html).'?'.(( qw( squeue condorq))[$mode]).'_'.
           ($ltim+$$_[1]).'" class="mon_default" target="WAYBACK">'.($$_[0]).
           '</a>' } ( grep {($$_[1] <= ($time - $ltim))} ([q(Next),+300],[q(Hour),+3600],[q(Day),+24*3600],[q(Week),+7*24*3600],[q(Month),+30*24*3600]))))[0]).
           '</DIV>'."\n";
  $tble .= "\t\t\t".'</td>'."\n";
